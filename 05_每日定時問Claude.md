...
| GitHub | **https://github.com/737540903/05_daily-claude-email**（Public，主帳號 **737540903**）。2026-06-29 由 `davidpaul9/daily-claude-email` 遷回主帳號並改名；舊 davidpaul9 repo 排程已停用、保留備份。 |
| 本機分支 / 遠端分支 | `main` → `main` |
...
- **GitHub 帳號 Actions 被停用（已解除）**：原帳號 `737540903` 曾在帳號層被停用 Actions（repo 按 Enable 報「Unable to enable」），當時改用 **davidpaul9** 才正常。**2026-06-29 確認 737540903 的 Actions 已恢復**（新 repo enabled、排程 active），故遷回主帳號。`gh` 用 PAT 登入需 `repo`、`workflow`、`read:org` scope；推此 repo 前若 active 帳號不是 737540903，先 `gh auth switch --user 737540903`。
...
_最後更新：2026-06-29（遷回主帳號 737540903 並改名 05_daily-claude-email、改 Public；停用 davidpaul9 舊 repo 排程避免重複寄信；機制 B 改 05:50）_
_2026-06-22（合併 05+06 為單一專案兩機制；機制 B routine 已驗證成功啟動 5 小時視窗，專案目的達成）_
